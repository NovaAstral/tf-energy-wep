...
SWEP.DrawAmmo = true
SWEP.WorldModel = "models/megarexfoc/w_foc_riot_cannon.mdl"
SWEP.ViewModel = "models/megarexfoc/viewmodels/c_foc_riot_cannon.mdl"

SWEP.Category = "Transformers Weapons"

SWEP.FOCEquip = Sound("cybertronian/tffusionequip.wav")
SWEP.FOCHolster = Sound("cybertronian/tffusionholster.wav")
...
function SWEP:Initialize()
    ...
    self.ShotColor = Color(177,0,231,215)
end
...
if(SERVER) then
    function SWEP:DoShooty()
        ...
        e:SetPos(shootpos)
        e:PrepareBullet(aimvector,multiply,8000,5)
        e:SetOwner(ply)
        e.Owner = ply
        e.Damage = 200
        e:Spawn()
        ...
        ply:EmitSound(Sound("cybertronian/tffusionshoot.wav"), 90, math.random(97, 103))
    end
...
timer.Simple(0.1, function() weapons.Register(SWEP,"tf_wep_shockblast", true) end) --Putting this in a timer stops bugs from happening if the weapon is given while the game is paused